...
- Start new projects by running `initialize_project` tool / `task-master init` or `parse_prd` / `task-master parse-prd --input='<prd-file.txt>'` (see [`taskmaster.instructions.md`](./taskmaster.instructions.md)) to generate initial tasks.json with tagged structure
...
- **Full CLI & MCP Support**: All tag management commands are available through both the CLI and MCP tools for you to use. Refer to [`taskmaster.instructions.md`](./taskmaster.instructions.md) for a full command list.
...
2.  **Environment Variables (`.env` / `mcp.json`):**
    ...
    - Available keys/variables: See `assets/env.example` or the Configuration section in the command reference (previously linked to [`taskmaster.instructions.md`](./taskmaster.instructions.md)).
...
7.  **Review & Update Instructions (Post-Implementation):**

    - Once the implementation for the subtask is functionally complete, review all code changes and the relevant chat history.
    - Identify any new or modified code patterns, conventions, or best practices established during the implementation.
    - Create new or update existing instructions following internal guidelines (previously linked to [`copilot_instructions.instructions.md`](./copilot_instructions.instructions.md) and [`self_improve.instructions.md`](./self_improve.instructions.md)).
